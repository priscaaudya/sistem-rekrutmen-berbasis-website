...
@section('container')
    <div class="card-header py-3">
        <p class="m-0 font-weight-bold " style="text-align: center">Detail Lamaran {{ $applicant->personal_identity->name}}</p>
    </div>
    <div class="float-right mr-5">
        <a href="/administration/{{ $applicant->id }}/add" class="btn btn-primary" data-toggle="modal" data-target="#modalcreate"> <i class="fas fa-plus-circle"> Administrasi</i></a>
    </div>
    ...
    <div class="card-body text-justify mt-3" >
        ...
        <div style="form-group row">
            <p>* Pas Foto <br></p>
            <img src="{{ asset('storage/' .$applicant->photo) }}" alt="{{ $applicant->personal_identity->name }}"  style="width:30%;height:40%" class="img-fluid">
        </div>
        <div class="form-group row mt-3">
            <p>* Surat Lamaran<br></p>
            <embed src="{{ asset('storage/' .$applicant->application_letter) }}" type="application/pdf" width="100%" height="600px" />
        </div>
        <div class="form-group row">
        ...
        <div class="form-group row">
            <p>*Ijazah <br></p>
            <embed src="{{ asset('storage/' .$applicant->certificate) }}" type="application/pdf" width="100%" height="600px" />
        </div>
        <div class="form-group row">
            <p>* Transkrip Nilai<br></p>
            <embed src="{{ asset('storage/' .$applicant->transcript) }}" type="application/pdf" width="100%" height="600px" />
        </div>
        @if ($applicant->str !== null)           
        <div class="form-group row">
            <p>* STR <br></p>
            <embed src="{{ asset('storage/' .$applicant->str) }}" type="application/pdf" width="100%" height="600px" />
        </div>
        @endif
        @if ($applicant->document !== null)
        <div class="form-group row">
            <p>* Dokumen Pendukung Lainnya<br></p>
            <embed src="{{ asset('storage/' .$applicant->document) }}" type="application/pdf" width="100%" height="600px" />
        </div>
        @endif
    </div>
@endsection
