...
@section('container')
    ...
    <ul class="nav nav-pills">
        ...
        <li><a href="/writtentest" class="nav-link active">Tes Tertulis</a>
        </li>
    ...
    <div style="float: right; margin-right:3%" >
        <a href="" class="btn btn-primary " data-toggle="modal" data-target="#modalconfirm"><i class="fas fa-plus-circle"> Pengumuman</i></a>
      </div>
              ...
              <tbody>
                @if($applicants->count())
                  ...
                  <tr align="center">
                      ...
                      <td>{{ Carbon\Carbon::parse($applicant->schedule->date)->format("d/m/Y") }} - {{ Carbon\Carbon::parse($applicant->schedule->time)->format("G.i") }}</td>
                      <td>@if ($applicant->attendance == '1') Hadir @elseif($applicant->attendance == '0') Tidak Hadir @else @endif</td>
                      <td>{{ $applicant->score }}</td>
                      <td> @if ($applicant->status == '1') Memenuhi @elseif($applicant->status == '0') Tidak Memenuhi @else @endif</td>
                      <td>
                          <a href="/writtentest/{{ $applicant->id }}" class="btn btn-info" data-toggle="modal" data-target="#modalshow{{ $applicant->id }}"><i class="fas fa-eye"></i></a>
                          <a href="/writtentest/{{ $applicant->id }}/edit" class="btn btn-warning"  data-toggle="modal" data-target="#modaledit{{ $applicant->id }}"><i class="fas fa-plus"></i></a>
